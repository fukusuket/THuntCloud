...
def test_dashboard_filter_ids_unique() -> None:
    ...
    dashboard = load_dashboard()
    filter_configs = (
        dashboard.get("metadata", {}).get("native_filter_configuration", [])
    )
    ids = [f["id"] for f in filter_configs]
    assert len(ids) == len(set(ids)), "Duplicate filter IDs in native_filter_configuration"
...
def test_dashboard_has_tab_layout() -> None:
    ...
    tab_entries = [v for v in position.values() if isinstance(v, dict) and v.get("type") == "TAB"]
    assert len(tab_entries) >= 4, (
...
def test_new_chart_referenced_in_dashboard(fragment: str, label: str) -> None:
    ...
    assert uuid is not None, f"{label}: chart YAML file not found (fragment='{fragment}')"
    dashboard = load_dashboard()
    dashboard_uuids = get_chart_uuids_from_dashboard(dashboard)
    assert uuid in dashboard_uuids, f"{label} (uuid={uuid}) not referenced in dashboard.yaml"
...
@pytest.mark.parametrize("filter_id,description", [
...
def test_new_native_filter_exists(filter_id: str, description: str) -> None:
    ...
    assert filter_id in filter_ids, f"{description} (id='{filter_id}') not found in dashboard.yaml"
